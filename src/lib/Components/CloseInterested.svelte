<script lang="ts">
  import { onMount } from 'svelte';
	...
	import {mainStore,close_notinterested} from '../../stores/general'
...
async function onFormSubmit(): void {
  ...
  if(formData.comment==''){
   ...
   if(valid){
	var response = await general.closenotinterested(formValues)
    Object.assign(formData, {
        comment:""
    })
    if ($modalStore[0].response) $modalStore[0].response(formData);
     toastStore.trigger({message:$close_notinterested.message})
 modalStore.close();
    ...
    onMount(async () => {
      ...
      Object.assign(formData, {
        ...
        types:$modalStore[0].props.types,
        author:$modalStore[0].props.author
        })
    }
    if($modalStore[0].props.types === "matching"){
      Object.assign(formData, {
        length:$modalStore[0].props.coursedata.length,
        // index:props.approvepropValues.index
      })
    }
    })
</script>
...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		...
		<header class={cHeader}>{$modalStore[0].title}-{$modalStore[0].props.name}</header>
